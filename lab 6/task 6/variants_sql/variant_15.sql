...
-- 5. "Вывести среднее количество студентов в группе и средний фонд финансирования кафедр."

SELECT t.avg_group_size, d.avg_dept_fund
FROM (
  SELECT AVG(g_count) AS avg_group_size FROM (
...
(SELECT AVG(dep.fund) AS avg_dept_fund FROM departments dep) d;
