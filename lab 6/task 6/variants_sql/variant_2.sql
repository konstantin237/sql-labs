...
-- 1. "Вывести среднее количество дисциплин на один факультет."

SELECT AVG(d_count) AS avg_disciplines_per_faculty
FROM (
  SELECT f.faculty_id, COUNT(d.disc_id) AS d_count
  FROM faculties f
  JOIN departments dep ON dep.faculty_id = f.faculty_id
  JOIN disciplines d ON d.dept_id = dep.dept_id
