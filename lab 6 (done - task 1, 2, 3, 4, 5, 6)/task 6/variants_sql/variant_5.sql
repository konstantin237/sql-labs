...
-- 4. "Вывести среднее количество студентов на курсе на каждом факультете."

-- Result: faculty_id, course, avg_students_on_course
SELECT faculty_id, course, AVG(group_size) AS avg_students_on_course
FROM (
  SELECT g.group_id, g.course, g.faculty_id, COUNT(s.student_id) AS group_size
  FROM groups g
  LEFT JOIN students s ON s.group_id = g.group_id
  GROUP BY g.group_id, g.course, g.faculty_id
) t
GROUP BY faculty_id, course
ORDER BY faculty_id, course;
